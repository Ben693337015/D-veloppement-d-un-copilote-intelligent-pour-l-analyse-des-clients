...
def _section_coude(diagnostics, k_choisi, silhouette_choisi, davies_bouldin_choisi=None, calinski_harabasz_choisi=None) -> str:
    """Diagnostic K-Means/GMM : graphique coude+silhouette, résumé du k
    retenu sur les TROIS métriques (silhouette, Davies-Bouldin,
    Calinski-Harabasz — cf. README §6), et table détaillée par k pour que
    le rapport reste vérifiable (Cadrage Tâche #10 : "les critères
    d'évaluation retenus" doivent être traçables, pas juste mentionnés)."""
    ...
    metriques_choisies = []
    if silhouette_choisi is not None:
        metriques_choisies.append(f"silhouette = {silhouette_choisi:.3f}")
    if davies_bouldin_choisi is not None:
        metriques_choisies.append(f"Davies-Bouldin = {davies_bouldin_choisi:.3f} (plus bas = meilleur)")
    if calinski_harabasz_choisi is not None:
        metriques_choisies.append(f"Calinski-Harabasz = {calinski_harabasz_choisi:.1f} (plus haut = meilleur)")
    resume = (
        f"<p><strong>k choisi : {k_choisi}</strong> ({', '.join(metriques_choisies)})</p>"
        if k_choisi is not None
        else ""
    )

    def _ligne_diag(d) -> str:
        db_val = f"{d.davies_bouldin:.3f}" if d.davies_bouldin is not None else "-"
        ch_val = f"{d.calinski_harabasz:.1f}" if d.calinski_harabasz is not None else "-"
        sil_val = f"{d.silhouette:.3f}" if d.silhouette is not None else "-"
        return (
            f"<tr><td>{d.k}</td><td>{d.inertie:.2f}</td><td>{sil_val}</td>"
            f"<td>{db_val}</td><td>{ch_val}</td></tr>"
        )

    table_detail = "".join(_ligne_diag(d) for d in diagnostics)
    return f"""
    <div class="section">
      ...
      <div class="carte">
        {resume}{corps}
        <table>
          <tr><th>k</th><th>Inertie</th><th>Silhouette</th><th>Davies-Bouldin</th><th>Calinski-Harabasz</th></tr>
          {table_detail or '<tr><td colspan="5">Aucun diagnostic disponible</td></tr>'}
        </table>
      </div>
    </div>
    """
    ...
    if "coude_silhouette" in sections:
        ...
        corps_html.append(
            _section_coude(
                diag.diagnostics,
                diag.k_choisi,
                diag.silhouette_choisi,
                diag.davies_bouldin_choisi,
                diag.calinski_harabasz_choisi,
            )
        )
