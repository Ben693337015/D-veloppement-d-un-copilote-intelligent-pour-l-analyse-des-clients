...
def get_derniere_maj_segmentation(db: Session):
    """Date de la dernière écriture de `run_full_pipeline` en base (colonne
    `clients.date_maj_segmentation`) — utilisée par le copilote pour signaler
    si la segmentation qu'il cite est fraîche ou potentiellement obsolète
    (ex. des transactions importées après le dernier calcul RFM)."""
    return db.query(func.max(Client.date_maj_segmentation)).scalar()
